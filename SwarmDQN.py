...
class SwarmDQN():
    ...
    def update(self):
        ...
        (non_final_mask, non_final_next_states, state, action, reward, l_x) = self.unpack_batch(batch)
        # == get Q(s, a) ==
        ...
        with torch.no_grad():
            ...
            # minimax values and indices
            # rowIdx = torch.argmin(torch.argmax(Q_mtx, 2), 1)[0]
            attackerValues, colIndices = Q_mtx.max(dim=-1)
            _, rowIdx = attackerValues.min(dim=-1)

            # defenders policy
            defendervalues = torch.from_numpy(np.array([Q_mtx[i, rowIdx[i], :] for i in range(64)]))
            _, colIdx = defendervalues.max(dim=-1)

            # defenderValues, _ = Q_mtx.max(dim=-1)
            # _, colIdx = defenderValues.max(dim=-1)
        ...
        # == Compute Expected Q values == Change to CQL based update and not DRABE
        expected_state_action_values = (
            torch.zeros(self.BATCH_SIZE).float().to(self.device)
        )
        non_terminal = torch.min(l_x[non_final_mask], self.GAMMA * next_state_values[non_final_mask])
        terminal = l_x

        # # # non-terminal state
        # expected_state_action_values[non_final_mask] = (
        #         non_terminal * self.GAMMA + terminal[non_final_mask] * (1 - self.GAMMA)
        # )
    ...
    def unpack_batch(self, batch):
        ...
        return (
            non_final_mask, non_final_next_states, state, action, reward, l_x
        )
    ...
    def learn(
            ...
            storeBest=True, outFolder="swarm", verbose=True
    ):
        """
        ...
        if storeModel:
            modelFolder = os.path.join(outFolder, f"model_{self.BETA}")
